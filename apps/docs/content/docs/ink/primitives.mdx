...
- Left and right arrows move the cursor within the buffer
- `Backspace` and `Delete` remove text relative to the cursor
- `Home` and `End` jump to buffer boundaries in single-line mode, or line boundaries in multi-line mode
- In multi-line mode, up and down arrows move between lines while preserving column when possible
- `Ctrl+A`, `Ctrl+E`, `Ctrl+J`, `Ctrl+W`, `Ctrl+U`, and `Ctrl+K` provide readline-style editing
- `Alt+B` and `Alt+F` move by word when the terminal reports them as meta-key input
- `Ctrl+J` always inserts a newline in multi-line mode
- `Shift+Enter` also inserts a newline in multi-line submit mode when the terminal reports it distinctly
